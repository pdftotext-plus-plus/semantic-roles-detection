def zip_lists(list_1, list_2):
    """
    Returns a list with length max(len(list_1), len(list_2)), where the i-th element is the
    concatenation of the i-th element in list_1 and the i-th element in list_2. On concatenating,
    the i-th elements in list_1 and list_2 are considered as lists, even if they are not. If an
    element is not a list (but a single element like an int or a string or any other object) it is
    considered as a single-element list. The i-th element in the returned list is then
    list_1[i] + list_2[i].
